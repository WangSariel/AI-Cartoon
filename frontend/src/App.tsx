...
import { ChevronLeft, ChevronRight, Plus, BookOpenText, Trash2, Home, MessageSquare, Image, PanelLeftClose, PanelLeftOpen, KeyRound, ExternalLink, X, Moon, Sun } from 'lucide-react';
...
const LS_THEME = 'lorevista.theme';
...
function ThemeToggle({ theme, onToggle, compact = false }: { theme: ThemeMode; onToggle: () => void; compact?: boolean }) {
  const isDark = theme === 'dark';
  return (
    <button
      onClick={onToggle}
      className="theme-toggle inline-flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium transition"
      title={isDark ? '切换到清新浅色模式' : '切换到沉静暗色模式'}
      aria-label={isDark ? '切换到清新浅色模式' : '切换到沉静暗色模式'}
    >
      {isDark ? <Sun size={14} /> : <Moon size={14} />}
      {!compact && (isDark ? '浅色' : '暗色')}
    </button>
  );
}
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className="space-y-5 px-5 py-5">
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between gap-3">
              <label className="text-xs font-medium text-gray-300">图片 API Key</label>
              <a
                href={IMAGE_API_CONSOLE_URL}
                target="_blank"
                rel="noreferrer"
...
function ApiKeyButton({ onClick, compact = false }: { onClick: () => void; compact?: boolean }) {
  ...
  const tipText =
    status === 'ok'
      ? '已配置 DeepSeek + 图片 API Key'
      : status === 'partial'
      ? `仅配置了 ${deepseek ? 'DeepSeek' : '图片'} API Key`
      : '未配置 API Key — 点击设置';
  return (
    <button
      onClick={onClick}
    ...
    return (
      <>
        <div className="fixed bottom-4 right-4 z-40 flex items-center gap-2">
          <ThemeToggle theme={theme} onToggle={toggleTheme} compact={isMobile} />
